...
# TTS imports are done inside the worker thread to ensure proper COM init on Windows
_tts_queue = queue.Queue(maxsize=32)
_last_spoken_by_text = {}
# per-phrase cooldown in seconds
cooldown = 0.0

def _tts_worker():
    # import pyttsx3 and initialize COM (pythoncom) here so SAPI runs from this thread
    try:
        import pythoncom
    ...
    try:
        ...
        print("[TTS worker] pyttsx3 import failed:", e, file=sys.stderr)
        # drain queue until sentinel then exit
        while True:
            t = _tts_queue.get()
    ...
    try:
        ...
        print("[TTS worker] engine init failed:", e, file=sys.stderr)
        engine = None

    while True:
        text = _tts_queue.get()  # blocks
        if text is None:
            _tts_queue.task_done()
            break
        try:
            if engine is None:
                print("[TTS worker] engine not available, dropped:", text)
            else:
                print(f"[TTS] speaking: {text}")
                engine.say(text)
                engine.runAndWait()
        except Exception as e:
            print(f"[TTS] error while speaking: {e}", file=sys.stderr)
        finally:
            _tts_queue.task_done()
...
def speak(text, force=False):
    """
    Enqueue text for speech. Uses per-text cooldown (so distinct phrases
    don't block each other). If pyttsx3/COM fail, queue operations will print errors.
    """
    now = time.time()
    last = _last_spoken_by_text.get(text, 0)
    if force or (now - last > cooldown):
        try:
            _tts_queue.put_nowait(text)
            _last_spoken_by_text[text] = now
            print(f"[speak] queued: {text}")
        except queue.Full:
            print("[speak] tts queue full, dropped:", text)
    else:
        print(f"[speak] suppressed by cooldown: {text}")
...
class Controller:
    ...
    # previous-state flags for edge detection (announce on gesture start only)
    prev_scrolling_up = False
    prev_scrolling_down = False
    prev_zoomming_in = False
    prev_zoomming_out = False
    ...
    def detect_scrolling():
        ...
        if scrolling_up:
            ...
            print("Scrolling UP")
            if not Controller.prev_scrolling_up:
                speak("Scrolling up")
        # update prev flag for scrolling up
        if not scrolling_up and Controller.prev_scrolling_up:
            Controller.prev_scrolling_up = False
        elif scrolling_up:
            Controller.prev_scrolling_up = True
        ...
        if scrolling_down:
            ...
            if not Controller.prev_scrolling_down:
                speak("Scrolling Down")
        # update prev flag for scrolling down
        if not scrolling_down and Controller.prev_scrolling_down:
            Controller.prev_scrolling_down = False
        elif scrolling_down:
            Controller.prev_scrolling_down = True
    ...
    def detect_zoomming():
        ...
        if zoomming_out:
            ...
            print("Zooming Out")
            if not Controller.prev_zoomming_out:
                speak("Zooming Out")
        ...
        if zoomming_in:
            ...
            pyautogui.keyUp('ctrl')
            print("Zooming In")
            if not Controller.prev_zoomming_in:
                speak("Zooming In")

        # update/reset prev zoom flags
        if not zoomming and (Controller.prev_zoomming_in or Controller.prev_zoomming_out):
            Controller.prev_zoomming_in = False
            Controller.prev_zoomming_out = False
        else:
            Controller.prev_zoomming_in = zoomming_in
            Controller.prev_zoomming_out = zoomming_out
